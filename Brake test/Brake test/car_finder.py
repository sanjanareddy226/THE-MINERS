...
class Car:
    ...
    def draw(self, img, color=(255, 0, 0), thickness=2):
        if self.display:
            ...
            if self.has_position:
               #print(self.position.output()[0], self.position.speed()[0]*self.fps*3.6)
                our_logic(self.position.output()[0], self.position.speed()[0]*self.fps*3.6)
